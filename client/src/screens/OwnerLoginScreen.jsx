...
export default function OwnerLoginScreen() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--navy)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}
         onKeyDown={(e) => e.key === 'Enter' && !busy && submit()}>
      <div style={{ marginBottom: 28, textAlign: 'center' }}>
        <svg viewBox="0 0 100 100" style={{ width: 64, height: 64, display: 'block', margin: '0 auto 14px' }} aria-hidden="true">
          <circle cx="50" cy="50" r="45" fill="none" stroke="var(--gold)" strokeWidth="1.8" />
          <g transform="translate(50,50)">
            <path d="M 0,5 C -10,-8 -8,-36 0,-42 C 8,-36 10,-8 0,5 Z" fill="var(--gold)" />
            <path d="M 0,5 C -10,-8 -8,-36 0,-42 C 8,-36 10,-8 0,5 Z" fill="var(--gold)" opacity="0.82" transform="rotate(72)" />
            <path d="M 0,5 C -10,-8 -8,-36 0,-42 C 8,-36 10,-8 0,5 Z" fill="var(--gold)" opacity="0.62" transform="rotate(144)" />
            <path d="M 0,5 C -10,-8 -8,-36 0,-42 C 8,-36 10,-8 0,5 Z" fill="var(--gold)" opacity="0.62" transform="rotate(216)" />
            <path d="M 0,5 C -10,-8 -8,-36 0,-42 C 8,-36 10,-8 0,5 Z" fill="var(--gold)" opacity="0.82" transform="rotate(288)" />
            <circle cx="0" cy="0" r="9" fill="var(--navy)" /><circle cx="0" cy="0" r="5" fill="var(--gold)" />
          </g>
        </svg>
        <div style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: 36, fontWeight: 700, letterSpacing: '-0.5px' }}>
          <span style={{ color: 'white' }}>Siam</span><span style={{ color: 'var(--gold)' }}>EPOS</span>
        </div>
      ...
      <div style={card}>
        ...
        <button onClick={submit} disabled={busy || !email || !password} style={{
          ...
          background: busy ? 'rgba(255,255,255,0.08)' : (email && password) ? 'var(--gold)' : 'rgba(255,255,255,0.06)',
          color: (email && password && !busy) ? 'var(--navy)' : 'rgba(255,255,255,0.28)',
