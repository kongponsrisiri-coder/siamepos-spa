...
const MODEL = process.env.TREATWELL_AI_MODEL || 'claude-sonnet-4-6';
...
const SCHEMA = {
  ...
  properties: {
    ...
    ref:           { type: ['string', 'null'], description: 'Treatwell order reference, e.g. T2185537204' },
    ...
    treatment:     { type: ['string', 'null'], description: 'treatment / product name' },
...
const PROMPT = `You extract booking details from a forwarded Treatwell venue email (English or Thai).
Rules:
- The "ref" is the Treatwell order reference (a T followed by digits) and appears in every email.
...
/**
 * @returns parser-shaped object { ok, action, ref, name, email, phone, treatment,
 *   durationMin, date, time, startLocal, room, price, prepaid, confidence, missing, via:'ai' }
 *   or { ok:false, reason }.
 */
async function extractBookingWithAI({ subject = '', text = '' } = {}) {
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: MODEL,
        max_tokens: 1024,
        tools: [{ name: 'record_booking', description: 'Record the extracted Treatwell booking', input_schema: SCHEMA }],
