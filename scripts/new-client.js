...
function buildSheet(d, now) {
  ...
  const envBlock = [
    ...
    ``,
    `# --- SHARED SiamEPOS keys: identical on every cloud. COPY the values from`,
    `#     an existing spa cloud (e.g. "Highbury Spa" / "Siam-Spa" on Railway).`,
    `#     Without these: no booking emails, no AI concierge, no SMS. ---`,
    `BREVO_API_KEY=            # email confirmations (Brevo)`,
    `ANTHROPIC_API_KEY=        # AI web-chat + WhatsApp concierge`,
    `TWILIO_ACCOUNT_SID=       # booking-confirmation SMS`,
    `TWILIO_AUTH_TOKEN=`,
    `TWILIO_FROM=+447861932999`,
    ``,
